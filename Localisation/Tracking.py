...
class Tracker:
    ...
    def computeHomography(self, targetPoints, observedPoints, minMatches = 12, weight = 0.4):
        ...
        if len(observedPoints) > minMatches:
            ...
            homography, _ = cv.findHomography(targetPoints, observedPoints, cv.RANSAC)

            if len(self.homography) == 0:
                self.homography = homography

            cv.accumulateWeighted(homography, self.homography, weight)
    ...
    def display(self, cameraId = 0, minMatches = 12, weight = 0.4, maxframeLost = 5, matchingRatio = 0.75):
        capture = cv.VideoCapture(cameraId)
        cv.namedWindow('Tracked plane', cv.WINDOW_NORMAL)

        # create trackbars
        cv.createTrackbar('Matching Ratio (%)', 'Tracked plane', int(matchingRatio * 100), 100, nullFunction)
