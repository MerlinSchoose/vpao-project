...
class CalibrationBase:
    ...
    def detect(self, frame, refine = False):
        patternFound, corners = False, None

        # Find the chessboard corners or circle grid centers       
        if self.patternType == 'chessboard':
            # Implement findChessboardCorners here
            patternFound, corners = (
                cv.findChessboardCorners(frame, (self.cols, self.rows), None)
            )
            ...
            patternFound, corners = (
                cv.findCirclesGrid(frame, (self.cols, self.rows), cv.CALIB_CB_SYMMETRIC_GRID)
            )

        elif self.patternType == 'asymmetric_circles':
            # Implement findCirclesGrid here
            patternFound, corners = (
                cv.findCirclesGrid(frame, (self.cols, self.rows), cv.CALIB_CB_ASYMMETRIC_GRID)
            )
...
class MonoCalibration(CalibrationBase):      
    ...
    def calibrate(self, framesPath = 'results/acquired'):
        ...
        (self.rms, self.cameraMatrix, self.distCoeffs, self.rvecs, self.tvecs,
         self.stdDeviationsIntrinsics, self.stdDeviationsExtrinsics, self.perViewErrors) = (
...
class StereoCalibration(CalibrationBase):        
    def calibrate(self, framesPathLeft = 'data/stereo/calibration/left', framesPathRight  = 'data/stereo/calibration/right'):        
        ...
        (self.rms, self.cameraMatrixLeft, self.distCoeffsLeft, self.cameraMatrixRight, self.distCoeffsRight,
         self.R, self.T, self.E, self.F, self.perViewErrors) = (
            cv.stereoCalibrateExtended(
                objectPointsLeft, imgPointsLeft, imgPointsRight, None, None, None, None, imageSizeLeft, None, None, flags=cv.CALIB_ZERO_DISPARITY+cv.CALIB_FIX_K3
            )
        )
