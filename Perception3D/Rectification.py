    ...
    def computeCorrectionMaps(self, alpha = 1.0, xRatio = 1, yRatio = 1):
        ...
        newCameraMatrix, _ = (
            cv.getOptimalNewCameraMatrix(
                self.cameraMatrix, self.distCoeffs, newImageSize, alpha
            )
        )
        ...
        self.map1, self.map2 = (
            cv.initUndistortRectifyMap(
                self.cameraMatrix, self.distCoeffs, None, newCameraMatrix, newImageSize, cv.CV_32FC1
            )
        )

    def rectify(self, frame):
        # Implement remap here
...
class StereoRectification:
    ...
    def computeCorrectionMaps(self, alpha = 1.0, ratio = 1):
        ...
        RLeft, RRight, PLeft, PRight, self.Q, _, _ = (
            cv.stereoRectify(self.cameraMatrixLeft, self.distCoeffsLeft, self.cameraMatrixRight, self.distCoeffsRight, self.imageSize, self.R, self.T)
        )
