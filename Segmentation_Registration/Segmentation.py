...
class Segmentation:
    ...
    def removeOutliers(self, display = False):
        ...
        cl, ind = self.pointCloud.remove_statistical_outlier(nb_neighbors=100, std_ratio=.6)
    ...
    def computeNormals(self, normalize = True, alignVector = [] ):
        ...
        self.pointCloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(1, 5))

        alignVector = np.asarray(alignVector)
        if alignVector.size == 3:
            # Implement normals alignement here
            self.pointCloud.orient_normals_to_align_with_direction(orientation_reference=alignVector)
    ...
    def alignFloor(self):
        ...
        rotationMatrix = rotationMatrixFromVectors(self.floorNormal, np.array([0, 1, 0]))
        self.pointCloud.rotate(rotationMatrix, center=(0, 0, 0))
    ...
    def removeFloor(self, bins = 10):
        ...
        maxY = edgesY[np.argmax(histY)+2]

        for i in range(len(xyz)):
            if xyz[i, 1] > maxY:
                newXYZ = np.append(newXYZ, [xyz[i]], axis = 0)
